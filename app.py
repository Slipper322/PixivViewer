    ...
    try:
        ...
        try:
            with open(CONFIG_FILE, 'w') as f:
                json.dump({"PHPSESSID": "ВАША_PHPSESSID_КУКА_СЮДА"}, f, indent=4)
        ...
        if data.get('body', {}).get('thumbnails', {}).get('illust'):
            for item in data['body']['thumbnails']['illust']:
                ...
                illusts_meta.append({
                    ...
                    # 'author_name': item.get('userName'), # userName есть в этом ответе
                    # 'author_id': str(item.get('userId')) if item.get('userId') else None # userId тоже
                })
...
def image_proxy():
    ...
    try:
        ...
        pixiv_response = requests.get(
            ...
            stream=True,
            timeout=25 # Увеличим таймаут для больших изображений
        )
        ...
        try:
            ...
            if original_filename_match:
                potential_filename = original_filename_match.group(1)
                # Убираем возможные query-параметры из имени, если они как-то попали (хотя re ищет в path)
                if '?' in potential_filename:
                    potential_filename = potential_filename.split('?')[0]
                
                # Проверяем, что имя файла содержит ID_p<номер>.<расширение>
                # Это эвристика, т.к. имя может быть и другим для превью
                if re.match(r'\d+_p\d+\.\w+', potential_filename) or re.match(r'\d+\.\w+', potential_filename) : # Для одиночных изображений без _p
                    filename = potential_filename
                else:
                    app.logger.warning(f"PROXY: Извлеченное имя '{potential_filename}' не соответствует ожидаемому формату ID_pX.ext. Используется стандартное имя.")
        ...
        if data.get('body') and isinstance(data['body'].get('works'), list):
            ...
            for item in raw_illust_list:
                ...
                illusts_meta.append({
                    ...
                    'width': item.get('width'), 
                    'height': item.get('height'),
                    # 'author_name': item.get('userName'), # userName есть
                    # 'author_id': str(item.get('userId')) if item.get('userId') else None # userId есть
                })
...
if __name__ == '__main__':
    ...
    app.run(debug=True, host='0.0.0.0', use_reloader=True) # use_reloader=True для удобства разработки